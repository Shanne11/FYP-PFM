...
Federated Baseline: FedProx (B5)
             │
             ▼
Mitigates Non-IID Weight Fluctuations
             │
             ▼
Proposed Framework:
...
FYP-PFM/
├── data/
│   ├── preprocess.py               # Handles missing values, deduplication, and data sanitization
│   ├── split_clients.py            # Shards the global clean data into 150 client user profiles
│   └── clients/                    # Holds localized client data (U001.csv to U150.csv)
│
├── dataset/
...
│
├── models/
│   ├── mlp.py                      # Multi-Layer Perceptron for federated learning
│   ├── note_model.py               # Random Forest implementation for Baseline 3
│   └── federated/
│       ├── client.py               # Federated client training workflow
│       ├── server.py               # Central server coordinator
│       └── aggregation.py          # FedAvg aggregation algorithm
│
├── utils/
│   ├── feature_engineering.py      # Feature preprocessing pipeline
│   ├── text_processing.py          # TF-IDF feature extraction
